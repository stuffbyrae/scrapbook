...
gd_open_gif(SDFile *fd)
{
	...
	if (memcmp(sigver, "GIF", 3) != 0) {
		pd->system->error("Not a GIF file");
		goto fail;
	}
	/* Version */
	read(fd, sigver, 3);
	if (memcmp(sigver, "89a", 3) != 0 && memcmp(sigver, "87a", 3) != 0) {
		pd->system->error("Incorrect GIF version header");
		goto fail;
	}
	/* Width x Height */
	width  = read_num(fd);
	...
	/* Presence of GCT */
	if (!(fdsz & 0x80)) {
		pd->system->error("No global color table");
		goto fail;
	}
	/* Color Space's Depth */
	depth = ((fdsz >> 4) & 7) + 1;
	...
	gif->palette = &gif->gct;
	gif->bgindex = bgidx;
	gif->frame = malloc(4 * width * height);
	if (!gif->frame) {
		free(gif);
		pd->system->error("Failed to allocate GIF canvas");
		goto fail;
	}
